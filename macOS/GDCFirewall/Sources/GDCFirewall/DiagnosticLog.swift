...
/// Logul de diagnostic al aplicației (Regulile 25 și 39).
///
/// Fiecare eveniment ajunge simultan în două locuri:
///   - unified log, subsistemul `dev.gordas.GDCFirewall` — `log show`, Console;
///   - `~/Library/Logs/GDCFirewall.log` — citibil direct în terminal, o linie
///     per eveniment: `timestamp NIVEL [categorie] mesaj`.
///
/// `scripts/logs.sh` le arată pe amândouă, plus starea extensiei. Extensia de
/// rețea (motorul LuLu, rulează ca root) scrie doar în unified log, sub
/// subsistemul `com.objective-see.lulu` — n-o modificăm; scriptul o include.
///
/// Nu se loghează parole, chei sau conținut de fișiere. Se loghează procesele
/// și destinațiile alertelor: sunt exact starea pe care o inspectezi.
...
/// diferite (XPC, UI, import) să nu se amestece.
private final class LogFile {
    static let shared = LogFile()

    private let queue = DispatchQueue(label: "dev.gordas.GDCFirewall.logfile", qos: .utility)
