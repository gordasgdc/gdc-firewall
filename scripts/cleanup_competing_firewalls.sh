#!/bin/bash
# Curățare profundă Little Snitch + LuLu — GDC Firewall rămâne singurul filtru.
#
#   ./scripts/cleanup_competing_firewalls.sh --dry-run   # doar inventarul; fără root, nu modifică nimic
#   sudo ./scripts/cleanup_competing_firewalls.sh        # inventar → confirmare → curățare → verificare
#   sudo ./scripts/cleanup_competing_firewalls.sh --yes  # fără confirmare
#
...
if [ "$(id -u)" = 0 ]; then
  ...
  if [ "$DRY" = 0 ]; then
    echo "Curățarea cere root:  sudo $0    (inventar fără modificări: $0 --dry-run)" >&2
    exit 1
  fi
  TUSER="$(id -un)"
fi
...
if [ "$DRY" = 1 ]; then
  out INFO "Previzualizare (--dry-run): nimic nu a fost modificat. Curățarea: sudo $0"
  exit 0
fi
...
while IFS=$'\t' read -r team bundle state; do
  ...
  if [ "$SIP_OFF" = 1 ]; then
    ...
    out INFO "  $bundle: se elimină prin Finder, odată cu aplicația (SIP activ)"
  fi
...
if cmp -s "$WORK/gdc.before" "$WORK/gdc.after"; then
