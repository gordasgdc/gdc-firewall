...
let ro = Guide(
    ...
    sections: [
        ...
        Section(heading: "4. Meniul din bara de sus", body: .bullets([
            ...
            "<b>„Se actualizează motorul de filtrare…”</b> — după instalarea unei versiuni noi, macOS înlocuiește filtrul vechi cu cel nou prin mecanismul lui oficial, fără parolă. Filtrul vechi te protejează până în clipa înlocuirii. De obicei durează câteva secunde, apoi meniul arată din nou „Protecție activă”.",
            "<b>„Repornește Mac-ul pentru a finaliza actualizarea”</b> — uneori macOS pornește filtrul nou fără legătura cu aplicația și îl conectează abia după o repornire. Apare o dată fereastra <b>„Repornește Mac-ul ca să finalizezi actualizarea filtrului”</b>; apasă <b>„Am înțeles”</b> și repornește Mac-ul când poți. Până atunci regulile tale se aplică în continuare, dar conexiunile noi, fără regulă, sunt blocate fără alertă — unele aplicații noi pot părea că nu au internet. După repornire, alertele revin singure.",
        ])),
        ...
        Section(heading: "9. Actualizarea", body: .note(
            "Aplicația verifică actualizările la fiecare pornire. Fereastra are două butoane: <b>„Actualizează acum”</b> descarcă și instalează singur noua versiune — îți cere parola de administrator, apoi aplicația repornește; <b>„Mai târziu”</b> o închide până la versiunea următoare. Nu e o actualizare silențioasă în fundal: tu decizi când se întâmplă.<br/><br/>Există și fereastra <b>„Actualizare critică de securitate”</b>, când motorul de filtrare nu mai e susținut. Aceea nu are „Mai târziu” și reapare la fiecare pornire. După actualizare, aplicația nouă înlocuiește și filtrul, prin mecanismul oficial macOS (funcționează cu protecția sistemului, SIP, activată). Uneori macOS cere o repornire ca să termine înlocuirea — vezi „Repornește Mac-ul pentru a finaliza actualizarea” mai sus.")),
...
let en = Guide(
    ...
    sections: [
        ...
        Section(heading: "4. The menu bar menu", body: .bullets([
            ...
            "<b>“Updating the filtering engine…”</b> — after you install a new version, macOS replaces the old filter with the new one through its official mechanism, with no password. The old filter protects you until the moment of replacement. It usually takes a few seconds, then the menu shows “Protection on” again.",
            "<b>“Restart your Mac to finish the update”</b> — sometimes macOS starts the new filter without its link to the app and connects it only after a restart. The <b>“Restart your Mac to finish the filter update”</b> window appears once; click <b>“OK”</b> and restart your Mac when convenient. Until then your rules still apply, but new connections without a rule are blocked without an alert — some new apps may seem to have no internet. After the restart, alerts come back on their own.",
        ])),
...
let es = Guide(
    ...
    sections: [
        ...
        Section(heading: "4. El menú de la barra de menús", body: .bullets([
            ...
            "<b>«Actualizando el motor de filtrado…»</b> — tras instalar una versión nueva, macOS sustituye el filtro anterior por el nuevo mediante su mecanismo oficial, sin contraseña. El filtro anterior te protege hasta el momento de la sustitución. Suele tardar unos segundos; después el menú vuelve a mostrar «Protección activa».",
            "<b>«Reinicia el Mac para terminar la actualización»</b> — a veces macOS inicia el filtro nuevo sin su conexión con la app y solo lo conecta tras un reinicio. Aparece una vez la ventana <b>«Reinicia el Mac para terminar la actualización del filtro»</b>; pulsa <b>«Entendido»</b> y reinicia el Mac cuando puedas. Hasta entonces tus reglas se siguen aplicando, pero las conexiones nuevas sin regla se bloquean sin alerta — algunas apps nuevas pueden parecer sin internet. Tras el reinicio, las alertas vuelven solas.",
        ])),
